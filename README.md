***PLEASE NOTE: THIS PROGRAM IS CURRENTLY UNDERGOING REVISION AS OF OCTOBER 2016 TO IMPROVE FUNCTIONALITY AND ACCESSIBILITY.  PLEASE DO NOT ATTEMPT TO USE UNTIL FURTHER NOTICE.
...
-EP_MELTSParser: reads BSP/MORB MELTS output files in the working directory and extracts oxide wt% compositions (for the MORB modeling process) and the sum of solid alloy generated (for the BSP modeling process).  Also contains a data aligning feature to deal with matching massive amounts of data to a spreadsheet already formatted with data.
...
After AutoMELTS produces the MELTS input files and stores them to their respective "BSP_MELTS_Files" or "MORB_MELTS_Files" subdirectory, it will call up the alphaMELTS algorithm and enter in one input file at a time, capturing the 'alphaMELTS_tbl.txt' comprehensive output file and renaming it to the name of the star with a BSP or MORB tag.  The files will be moved to a "Completed_BSP_MELTS_Files" or "Completed_MORB_MELTS_Files" directory and reformatted to CSV for parsing convenience.  These CSV-formatted versions of the alphaMELTS outputs are what other components of the alphaMELTS suite will look for, so please transfer these over to your shared folder (if you are using a virtual machine) for further actions.  Again, please do not rename them.  Once calculations are complete and CSV-formatted files are created, the AutoMELTS script will automatically exit.

The user should note that AutoMELTS contains a built in kill timer, should the alphaMELTS algorithm enter an infinite loop.  If the calculation exceeds an reasonable amount of time for a calculation to complete, the process will be killed and AutoMELTS will move on to the next calculation.  However, it is not yet possible to kill the child process spawned by the run_alphamelts.command script.  Therefore, it is wise to run batches of <50 stars at a time in order to avoid hanging your system with too many open alphaMELTS tasks.
...
This script also contains a data alignment feature which is useful for transferring large amounts of data to an already formatted spreasheet.  This is a snappy way to see which stars did or did not have calculation failures.  After entering the 'align' prompt, the user will be asked for 'BSP', 'MORB', or 'universal'.  BSP and MORB work for data sizes generated by the BSP and MORB features in this script, but are actually redundant due to 'universal' being able to accept any data size.  Still, chose what you are most comforatble with.

The useage of this feature is perhaps best explained through an example.  Say you have the following stars in a spreadsheet:
...
And say that only Star1 and Star3 had successful calculations and had outputs written from other sections of the Exoplanet Pocketknife suite (say, for example, MORBMinusBSP outputs or even solid alloy sums given by this script.  The output file would look like this:
...
After running the alignment feature, an output titled 'Aligned_BSP_Outputs.csv' (for BSP), 'Aligned_MORB_Outputs.csv' (for MORB), or 'Aligned_Outputs.csv' (for universal) will appear (or be deleted and recreated if it already exists).  It will appear as follows:
...
Mind that the data can be of any CSV row length when using the 'universal' subfeature.  'BSP' and 'MORB' alignment subfeatures look for set data lengths.
...
**PLEASE NOTE: THIS SCRIPT IS A WORK IN PROGRESS AS CALCULATIONS ARE BEING DECIDED UPON.  THE FOLLOWING DESCRIPTION MAY NOT BE ENTIRELY CORRECT.**
